...
Travel the region on an overworld map; enter locations from it. Map view sits above the current World tab (region → location → scene drill-down). Implementation TBD — SVG or canvas overlay on a hand-drawn or generated map image.
...
Full-text search across NPCs, scenes, and locations. Priority: NPC name and role, scene taglines, location names.

## Audio Pipeline

Record session → `transcript.md` → automated debrief. See [session-workflows.md](session-workflows.md) for the transcript debrief workflow spec.
